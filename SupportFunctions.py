...
def get_myday(name, data, month_name=None):
    ...
    if month_name == None:
        myday = 'AjKyaUkhada'
        myday_marks = data[data['Task'] == myday].groupby('Student')['Points'].sum().sort_values(
        ...
        myday = 'AjKyaUkhada'
        myday_marks = data[(data['Task'] == myday) & (data['Month'] == month_name)].groupby('Student')[
...
def get_task(name, data, month_name=None):
    ...
    myday = 'AjKyaUkhada'
    if month_name == None:
        task_marks = data[(data['Task'] != knowledge) & (data['Task'] != myday)].groupby('Student')[
            'Points'].sum().sort_values(ascending=False).reset_index()
        ...
        task_marks = \
        data[(data['Task'] != knowledge) & (data['Task'] != myday) & (data['Month'] == month_name)].groupby('Student')[
            'Points'].sum().sort_values(ascending=False).reset_index()
        temp = task_marks[task_marks['Student'] == name]['Points'].values
        if len(temp) > 0:
            return temp[0]
...
def get_gyan(name, data, month_name=None):
    if month_name == None:
        knowledge_marks = data[data['Task'] == 'Knowledge Sharing'].groupby('Student')['Points'].sum().sort_values(
            ascending=False).reset_index()
        ...
        knowledge_marks = \
        data[(data['Task'] == 'Knowledge Sharing') & (data['Month'] == month_name)].groupby('Student')[
            'Points'].sum().sort_values(ascending=False).reset_index()
        temp = knowledge_marks[knowledge_marks['Student'] == name]['Points'].values
        if len(temp) > 0:
            return temp[0]
...
def get_count_late_submission(name, data, month_name=None):
    if month_name == None:
        count = data[data['Late Submission'] == 1].groupby('Student')['Late Submission'].count().sort_values(
            ascending=False).reset_index()
        ...
        count = data[(data['Late Submission'] == 1) & (data['Month'] == month_name)].groupby('Student')[
            'Late Submission'].count().reset_index()
        temp = count[count['Student'] == name]['Late Submission'].values
        if len(temp) > 0:
            return temp[0]
        else:
...
def get_count_task_won(name, data, month_name=None):
    if month_name == None:
        count = data[data['Task Winner'] == 1].groupby('Student')['Task Winner'].count().sort_values(
            ascending=False).reset_index()
        ...
        count = data[(data['Task Winner'] == 1) & (data['Month'] == month_name)].groupby('Student')[
            'Task Winner'].count().reset_index()
        temp = count[count['Student'] == name]['Task Winner'].values
        if len(temp) > 0:
            return temp[0]
        else:
