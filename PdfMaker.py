...
class PdfMaker:
    ...
    def generate_report_monthly(self, month, name):
        ...
        if self.month_check(month, a.months):
            ...
            # greeting
            nameText = """<font face=Helvetica><b>Hi <i>{}</i>, This is your monthly report for the month of {}.</b></font>""".format(
                name, month)
            ...
            # knowledge
            knowledge_sharing_count = a.student_knowledge_marks(name=name, month_name=month)[0][
                'Number of days'].values.tolist()
            if len(knowledge_sharing_count) > 0:
                knowledge_sharing_count = knowledge_sharing_count[0]
            else:
                knowledge_sharing_count = 0
            initialColumns = ['Rank', 'Total Marks', 'MyDay Count', 'Knowledge Sharing Count', 'Late Submissions',
                              'Task\'s Won']

            # Creating first table and appending to flowables
            initial_tbl_data = [initialColumns,
                                [rank, total_marks, myday_count, knowledge_sharing_count, late_submissions, task_won]]
            ...
            # Tasks
            text = Paragraph('<para align=left><font face=helvetica size=15><b>Tasks:</b></font></para>',
            ...
            text = Paragraph('<para align=left><font face=helvetica size=15><b>Leaderboard:</b></font></para>',
                             style=style['Heading2'])
            flowables.append(text)
            leaderboard_monthly_table = self.row_table_builder2(
                dataframe=a.total_marks_leaderboard(month_name=month)[0])
            ...
            flowables.append(PageBreak())
            text = Paragraph(
                '<para align=left><font face=helvetica size=15><b>Previous Month\'s Leaderboard:</b></font></para>',
                style=style['Heading2'])
            flowables.append(text)
            flowables.append(Spacer(1, 20))
            leaderboard_prev_month_data = self.row_table_builder2(
                dataframe=a.total_marks_leaderboard(month_name=previous_month[month])[0])
