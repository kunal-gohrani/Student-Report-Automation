...
class Analysis:
    ...
    def total_marks_leaderboard(self, month_name=None):
        ...
        if month_name == None:
            ...
            total_marks['Knowledge Sharing Marks'] = total_marks['Student'].apply(SupportFunctions.get_gyan,
                                                                                  args=[data])
            total_marks['Late Submissions'] = total_marks['Student'].apply(SupportFunctions.get_count_late_submission,
                                                                           args=[data])
            ...
            total_marks = data[data['Month'] == month_name].groupby('Student')['Points'].sum().sort_values(
                ascending=False).reset_index()
            total_marks.rename(columns={'Points': 'Marks'})
            total_marks['Total Marks'] = total_marks['Student'].apply(SupportFunctions.get_highest_marks,
                                                                      args=[temp, month_name])
            total_marks['Task Marks'] = total_marks['Student'].apply(SupportFunctions.get_task, args=[temp, month_name])
            total_marks['MyDay Marks'] = total_marks['Student'].apply(SupportFunctions.get_myday,
                                                                      args=[temp, month_name])
            total_marks['Knowledge Sharing Marks'] = total_marks['Student'].apply(SupportFunctions.get_gyan,
                                                                                  args=[temp, month_name])
            total_marks['Late Submissions'] = total_marks['Student'].apply(SupportFunctions.get_count_late_submission,
                                                                           args=[temp, month_name])
            total_marks['Tasks Won'] = total_marks['Student'].apply(SupportFunctions.get_count_task_won,
                                                                    args=[temp, month_name])
            return tuple((total_marks, total_marks.style.hide_index()))
        ...
        if month_name != None:
            if month_name in self.months:
                task_marks = data[(data['Task'] != 'Knowledge Sharing') & (data['Task'] != 'AjKyaUkhada') & (
                            data['Student'] == name) & (data['Month'] == month_name)]
            ...
            task_marks = data[
                (data['Task'] != 'Knowledge Sharing') & (data['Task'] != 'AjKyaUkhada') & (data['Student'] == name)]
    ...
    def student_myday(self, name=None, month_name=None):
        ...
        if name == None:
            raise ValueError('Input a name')
        if month_name == None:
            myday_marks = data[(data['Task'] == myday) & (data['Student'] == name)].groupby('Student')[
                'Points'].sum().sort_values(ascending=False).reset_index()
            myday_marks.rename(columns={'Points': 'Myday Marks'}, inplace=True)
            myday_count = data[(data['Task'] == myday) & (data['Student'] == name)].groupby('Student')[
                'Points'].count().sort_values(ascending=False).reset_index()
            ...
            myday_marks = \
            data[(data['Month'] == month_name) & (data['Task'] == myday) & (data['Student'] == name)].groupby(
                'Student')['Points'].sum().sort_values(ascending=False).reset_index()
            myday_marks.rename(columns={'Points': 'Myday Marks'}, inplace=True)
            myday_count = \
            data[(data['Month'] == month_name) & (data['Task'] == myday) & (data['Student'] == name)].groupby(
                'Student')['Points'].count().sort_values(ascending=False).reset_index()
            myday_count.rename(columns={'Points': 'Number of days'}, inplace=True)
            myday_data = myday_marks.merge(myday_count, on='Student')
            return tuple((myday_data, myday_data.style.hide_index()))
    ...
    def student_knowledge_marks(self, name=None, month_name=None):
        ...
        if name == None:
            raise ValueError('Input a name')
        if month_name == None:
            knowledge_marks = data[(data['Task'] == knowledge) & (data['Student'] == name)].groupby('Student')[
                'Points'].sum().sort_values(ascending=False).reset_index()
            knowledge_marks.rename(columns={'Points': 'Knowledge Sharing Marks'}, inplace=True)
            knowledge_count = data[(data['Task'] == knowledge) & (data['Student'] == name)].groupby('Student')[
                'Points'].count().sort_values(ascending=False).reset_index()
            ...
            knowledge_marks = \
            data[(data['Month'] == month_name) & (data['Task'] == knowledge) & (data['Student'] == name)].groupby(
                'Student')['Points'].sum().sort_values(ascending=False).reset_index()
            knowledge_marks.rename(columns={'Points': 'Knowledge Sharing Marks'}, inplace=True)
            knowledge_count = \
            data[(data['Month'] == month_name) & (data['Task'] == knowledge) & (data['Student'] == name)].groupby(
