...
def knowledgeSharingCleaner(data):
    idx = np.array([])
    knowledge_sharing_days = {'Monday': ['Kunal', 'Bhavna', 'Apurwa'], 'Tuesday': ['Chandrima', 'Purbita', 'Prasoon'],
                              'Wednesday': ['Roumyak', 'Kaushal', 'Ujjainee'],
                              'Thursday': ['Siddhishikha', 'Dipam', 'Shakib'], 'Friday': ['Sonali', 'Durga', 'Sonali'],
                              'Saturday': ['Swaastick', 'Sharika', 'Vishal'], 'Sunday': ['Anjali', 'Arya', 'Surabhi']}
    date_month_dayname_unique = data[['Date', 'Month', 'DayName']].drop_duplicates()
    for date, month, dayname in zip(date_month_dayname_unique['Date'].values.tolist(),
                                    date_month_dayname_unique['Month'].values.tolist(),
                                    date_month_dayname_unique['DayName'].values.tolist()):
